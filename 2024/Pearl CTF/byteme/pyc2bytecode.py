    ...
    try:
        with open(pathofpyc, "rb") as f:
            ...
            if major_version == 2:  # Python version is "2" | Same for All Versions
                ...
                print(
                    "[+] Timestamp: " + time.ctime(struct.unpack("L", timestamp_val)[0])
                )
                ...
                if minor_version < 3:  # For python version less than Python3.3
                    ...
                    print(
                        "[+] Timestamp: "
                        + time.ctime(struct.unpack("L", timestamp_val)[0])
                    )
                    ...
                    filesize_bytes = struct.unpack("<L", filesize_val)[
                        0
                    ]  # Convert into little endian unsigned long integer
                ...
                # If Python version is >= 3.7
                    ...
                    if (
                        bit_field_val.hex()[7] == "0"
                    ):  # Check if the last bit = 0 then Timestamp-based PYC
