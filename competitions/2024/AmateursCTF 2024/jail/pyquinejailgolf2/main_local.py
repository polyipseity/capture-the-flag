...
with TemporaryDirectory(ignore_cleanup_errors=True) as workdir:
...
try:exec(program, new_builtins, new_builtins)
        ...
        handle = __import__("subprocess").run(
            "python3 runtime/external_run.py",
            shell=True,
            capture_output=True,
            encoding="utf-8",
        )
